...
**Functional dependency**—multiple Stories share certain modules but don't block each other (e.g., "product search" and "product favorites" both depend on product list component). These Stories can enter micro-waterfall in parallel; if their SDD incremental updates touch the same module, human confirms no conflicts at Review Checkpoint. The same rule extends to Behavior Specs (FB-021): NNN allocation counts all active deltas (no ADDED collisions); two Stories MODIFYing the same Requirement is a Review Checkpoint conflict, and the later merger re-bases its MODIFIED text against the now-current spec before merging.
