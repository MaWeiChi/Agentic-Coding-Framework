...
| verify | Execute four checks: Completeness (every Requirement ID touched has a test via `Spec:` headers — scenario-exempted Requirements count via their `assertion_type: parameter` tests, Deferred/TBD-blocked ones fail until resolved; orphan reverse check: no test carries a `Spec:` ID absent from specs + active deltas; all Delta implemented), Correctness (tests pass, NFR met), Coherence (specs/SDD merged Deltas, contracts consistent, Constitution not violated), Security (no hardcoded secrets). On pass, merge `## Behavior Delta` → docs/specs/ and `## SDD Delta` → docs/sdd.md (skip `## Review Disclosure`): REMOVED → delete block + append `## Removed` tombstone + delete tests carrying that `Spec:` ID in this Story; MODIFIED/REMOVED target missing → stop, log `[High]` ISSUE, return to bdd (FB-021). Then move the delta file to `docs/deltas/archive/{YYYY-MM-DD}-US-{story}.md` |
...
| v0.20 | 2026-06-13 | FB-021: verify instruction gains the orphan-test reverse check, REMOVED tombstone semantics, and merge-target-missing handling (stop + ISSUE + return to bdd) |
| v0.21 | 2026-06-13 | Batch-verification fix: verify instruction adds the missing test-deletion half of REMOVED (delete tests carrying that `Spec:` ID in the removing Story) |
