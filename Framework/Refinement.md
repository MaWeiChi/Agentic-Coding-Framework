...
| v0.28 | 2026-06-13 | FB-021~025 batch adversarial verification (5 lenses): 1 blocker fixed — Examples' MODIFIED block now restates the full Requirement (whole-block replacement; delta/spec coherent); Protocol verify adds REMOVED test-deletion; Lifecycle parallel-Story clause restores re-run-Coherence; conflict-scan scope extended to other active deltas at all sites; SKILL "When This Skill Applies" aligned to FB-022 `.ai/` semantics; orphan check tombstone clause; boundary-flavored example scenario relabeled; collapse rule pinned to `###`-inline form; abandoned-delta NNN interaction noted; lint hardened (missing-README-row check, bold-form ACF regex, changelog-row exclusion, Examples tracked). Templates v0.21, Lifecycle v0.19, Protocol v0.21, Examples v0.2 |
